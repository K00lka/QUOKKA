...
const App: React.FC = () => {
  ...
  const data:Record<string,{description:string,image:string,Question?:string,Baton_1:Button,Baton_2?:Button, Baton_3?:Button}>={
    ...
    Fatal:{
      ...
      image: "./QUOKKA/Game_over.jpeg",
      
      Baton_1:{
        tekst: "Spróbuj ponownie",
        direct_to: () => {setScene("Scena_1");setGałązki(false)}
      }
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-[#323232] text-[#FAC452] font-mono">
      {/* Main Content */}
      <div className="flex flex-1 h-[calc(100%-150px)] flex-row items-center justify-center p-5 gap-5 w-full">
        {/* Left Section */}
        <div className="flex h-full w-[400px] bg-[#6B2D59] p-5 border-2 border-[#FAC452] rounded-lg">
          <p className="p-2">
        ...
        {/* Image Section */}
        <div className="flex aspect-[4/3] h-full bg-[#323232] border-2 border-[#FAC452] rounded-lg items-center justify-center">
        <img src={data[scene].image} className="w-full h-full"></img>
        </div>
      </div>

      {/* Footer Section */}
      <div className="bg-[#893A75] p-5 flex items-center justify-center flex-col w-full h-[150px]">
      {data[scene].Question}
